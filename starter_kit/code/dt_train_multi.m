...
% Usage:
%
%    tree = dt_train(X, Z, DEPTH_LIMIT)
%
% Returns a tree of maximum depth DEPTH_LIMIT learned using the ID3
% algorithm. Assumes X is a N x D matrix of N examples with D features. Z
...
%   node.value - 1 x K vector of P(Z==K) as predicted by this node
...
Z = bsxfun(@eq, Y, repmat([1:max(Y)],length(Y),1));
...
root = split_node(X, Z, Xrange, mean(Z,1), 1:size(Xrange, 2), 0, depth_limit);
...
if depth == depth_limit || sum(mean(Z,1)==1) == 1 || size(Z,1) <= 1 || numel(colidx) == 0
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if numel(Z) == 0
    ...
    %fprintf('depth %d [%d/%d]: Leaf node: = %s\n', depth, sum(Z==0), sum(Z==1), mat2str(node.value));
    return;
end
...
%fprintf('depth %d [%d]: Split on feature %d <= %.2f w/ IG = %.2g (L/R = %d/%d)\n', depth, size(Z,1), node.fidx, node.fval, max_ig, numel(leftidx), numel(rightidx));
