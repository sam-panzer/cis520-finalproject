function [f_output f_output_votes]= eval_Stochastic_Bosque(Data,Random_Forest,varargin)
...
f_output_votes = zeros(size(Data,1),10);
for i = 1 : size(Data,1)
    
    indicators = (repmat(f_votes(:,i),1,10) == repmat([1:10],size(f_votes,1),1));
    f_output_votes(i,:) = sum(indicators,1);
end
